...
4. Railway automatically detects the included `Dockerfile`, which installs the
   package from `src`. Clear any old custom command such as `start.sh`. If a Start
   Command override is present, set it to:
